...
export type MetricAttempt = {
  ...
  occurredAt: string;
  cognitiveLevel?: "recognition" | "recall" | "application" | "transfer" | "synthesis";
};
...
export type ExerciseAnalytics = {
  ...
  observedDifficultyStatus: "too_easy" | "appropriate" | "challenging" | "too_difficult" | "insufficient_data";
...
const percentile = (values: number[], point: number) => {
  ...
  return Math.round(sorted[Math.min(sorted.length - 1, Math.floor(point * sorted.length))]);
};
...
export function calculateExerciseAnalytics(
  ...
  const successfulRetryUsers = new Set(retries.filter((attempt) => attempt.isCorrect).map((attempt) => attempt.userId)).size;
  const usersWithRetries = new Set(retries.map((attempt) => attempt.userId)).size;
  const eventualByUser = new Map<string, boolean>();
  attempts.forEach((attempt) => eventualByUser.set(attempt.userId, (eventualByUser.get(attempt.userId) ?? false) || attempt.isCorrect));
  const firstAccuracy = ratio(first.filter((attempt) => attempt.isCorrect).length, first.length);
  const eventualAccuracy = ratio([...eventualByUser.values()].filter(Boolean).length, eventualByUser.size);
  const hintUsage = ratio(attempts.filter((attempt) => attempt.hintsUsed > 0).length, attempts.length);
  ...
  const discrimination = high.length >= 5 && low.length >= 5
    ? ratio(high.filter((attempt) => attempt.isCorrect).length, high.length) - ratio(low.filter((attempt) => attempt.isCorrect).length, low.length)
    : null;
  const transfer = first.filter((attempt) => attempt.cognitiveLevel === "transfer" || attempt.cognitiveLevel === "synthesis");
  const transferScore = transfer.length >= 5
    ? ratio(transfer.filter((attempt) => attempt.isCorrect).length, transfer.length)
    : null;
  const sufficient = attempts.length >= minimumAttempts && uniqueUsers >= minimumUsers;
  const masteryAdjustment = ratio(first.filter((attempt) => attempt.masteryBefore === "low").length, first.length) * 8;
  const responseComponent = Math.min(15, percentile(responseTimes, 0.5) / 20_000 * 15);
  const retryRate = ratio(usersWithRetries, uniqueUsers);
  const difficultyScore = Math.max(0, Math.min(100,
    (1 - firstAccuracy) * 60 + hintUsage * 15 + retryRate * 10 + responseComponent - masteryAdjustment,
  ));
  ...
  const ambiguitySignals = [
    ...
    high.length >= 5 && ratio(high.filter((attempt) => attempt.isCorrect).length, high.length) < 0.55,
  ].filter(Boolean).length;
  const ambiguityRisk = Math.min(100, ambiguitySignals * 18);
  const feedbackTotal = (signals.helpfulFeedback ?? 0) + (signals.unhelpfulFeedback ?? 0);
  const feedbackEffectiveness = feedbackTotal >= 5
    ? ratio(signals.helpfulFeedback ?? 0, feedbackTotal)
    : 0.5;
  const difficultyFit = observedDifficultyStatus === "appropriate" || observedDifficultyStatus === "challenging" ? 1 : 0.5;
  const discriminationQuality = discrimination === null ? 0.5 : Math.max(0, Math.min(1, (discrimination + 0.2) / 0.6));
  ...
  const qualityScore = sufficient
    ? Math.max(0, Math.min(100,
      breakdown.structuralValidity + breakdown.appropriateDifficulty + breakdown.discriminationQuality +
      breakdown.feedbackEffectiveness + breakdown.retentionContribution + breakdown.completionHealth -
      breakdown.ambiguityPenalty - breakdown.abandonmentPenalty,
    ))
    : null;
  const qualityStatus: ExerciseQualityStatus = qualityScore === null
    ? "insufficient_data"
    : qualityScore >= 85
      ? "healthy"
      : qualityScore >= 70
  ...
  return {
    ...
    averageScore: round(ratio(attempts.reduce((sum, attempt) => sum + attempt.score, 0), attempts.length), 2),
    ...
    qualityStatus,
    qualityBreakdown: Object.fromEntries(Object.entries(breakdown).map(([key, value]) => [key, round(value, 2)])),
...
export function classifyDistractor(input: {
  ...
  if (input.sampleSize < input.minimumSample) return "insufficient_data" as const;
  if (input.selectionRate < 0.03) return "unused" as const;
  if (input.highMasteryRate > 0.3 && Math.abs(input.highMasteryRate - input.lowMasteryRate) < 0.08)
...
export function calculateRetention(immediate: number[], delayed: number[], minimumSample = 10) {
  if (immediate.length < minimumSample || delayed.length < minimumSample)
    return { dataStatus: "insufficient_data" as const, immediateAccuracy: null, delayedAccuracy: null, retentionRate: null, retentionDecay: null };
  const immediateAccuracy = ratio(immediate.filter((score) => score >= 0.7).length, immediate.length);
  const delayedAccuracy = ratio(delayed.filter((score) => score >= 0.7).length, delayed.length);
  return {
