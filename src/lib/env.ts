...
const serverEnvSchema = clientEnvSchema.extend({
  ...
  TRAINING_ROLEPLAY_MAX_TURNS: z.coerce.number().int().min(2).max(100).default(20),
  TRAINING_ROLEPLAY_MAX_DURATION_SECONDS: z.coerce.number().int().min(60).max(7200).default(1800),
  TRAINING_ROLEPLAY_MAX_CONCURRENT: z.coerce.number().int().min(1).max(5).default(1),
  TRAINING_ROLEPLAY_RATE_LIMIT_PER_MINUTE: z.coerce.number().int().min(1).max(60).default(8),
  TRAINING_ROLEPLAY_DAILY_EMERGENCY_LIMIT: z.coerce.number().int().min(1).max(500).default(20),
