...
export async function GET(request: NextRequest) {
  ...
  if (!access) return jsonError({ code: "FORBIDDEN", message: "No tienes permiso para consultar analítica." }, 403);
  return jsonData(await new TrainingAnalyticsService(access.service).dashboard());
}
