...
export async function GET(request: NextRequest) {
  if (
    ...
    return jsonError(
      { code: "FEATURE_DISABLED", message: "Las rutas no están disponibles." },
      404,
    );
  ...
  if (!context)
    return jsonError(
      {
        code: "UNAUTHORIZED",
        message: "Inicia sesión para consultar tus rutas.",
      },
      401,
    );
  const parsed = trainingPathFiltersSchema.safeParse(
    Object.fromEntries(new URL(request.url).searchParams),
  );
  if (!parsed.success)
    return jsonError(
      { code: "INVALID_FILTERS", message: "Revisa los filtros seleccionados." },
      400,
    );
  return jsonData(
    await getTrainingPathService().getPaths(context.userId, context.plan, {
      query: parsed.data.q,
